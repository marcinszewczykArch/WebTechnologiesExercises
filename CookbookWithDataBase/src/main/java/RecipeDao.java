...
public class RecipeDao {
    ...
    public RecipeDao() {
        try {
            this.connection = DriverManager.getConnection("jdbc:mysql://localhost:3306/cookbook", "root", "admin");
    ...
    void save(Recipe recipe) {
        final String sql = String.format(
                "INSERT INTO recipe (title, preptime, ingredients, description) " +
                "VALUES ('%s',%d,'%s','%s')",
    ...
    Optional<Recipe> findByTitle(String searchTitle) {
        final String sql = String.format("SELECT id, title, description, ingredients, preptime FROM recipe WHERE title='%s'",searchTitle);
    ...
    boolean delete(int id) {
        ...
        try(Statement statement = connection.createStatement()) {
            int updadedRows = statement.executeUpdate(sql);
            return updadedRows!=0;
        } catch(SQLException e) {
            throw new RuntimeException(e);
        }
    }
